...
CREATE TABLE bw_chat_message_ext (
    ...
    rel_docs TEXT[] NOT NULL,              -- 相关文档数组，存储多个文档标识符
    created_at BIGINT NOT NULL,            -- 创建时间，Unix 时间戳
    updated_at BIGINT NOT NULL             -- 更新时间，Unix 时间戳
);

CREATE UNIQUE INDEX idx_bw_chat_message_ext_session_id_message_id ON bw_chat_message_ext (session_id, message_id); -- 空间ID索引，提升按空间查询的速度
